...
def ranking():	
	connection  = db_connection()
	cursor = connection.cursor()
	cursor.execute("""SELECT name,playlist_id, ranking, today_followers FROM playlists WHERE id != 1 ORDER BY ranking ASC""")
...
def promotion_ranking():
	...
	query = 'SELECT today_followers , playlist_id from playlists WHERE id IN (%s) ORDER BY today_followers DESC' % ','.join('?' for i in ids)
